...
<?php include("./include/header.php") ?>
    ...
    echo 
        "<img id=\"banner\" src=\"" . $gameInfo["image"] . 
    ...
    echo "<div id=\"addToBookmark\">" .
        "<form method=\"POST\">"; //GET removes the already present gameID = x;

        if(isset($_SESSION["username"]) && isset($_SESSION["bookmark"])){
            if($_SESSION["bookmark"][$id]){
                // game is in bookmark
    ...
    if(isset($_SESSION["isAdmin"])){
        if(isset($_POST["removeGameID"])){
            removeGame($conn, $_POST["removeGameID"]);
            header("Location: ./index.php");
        }
        if($_SESSION["isAdmin"]){
            echo "<div id=\"removeGame\"><form method=\"POST\">
                <input type=\"hidden\" name=\"removeGameID\" value=\"". $id ."\">
    ...
    if(isset($_SESSION["userID"])){
        ...
        if(!hasReview($conn, $_SESSION["userID"], $id)){
            echo   "newReview.classList.toggle(\"hidden\");
                    ...
                    flipBool = !flipBool;};showNewReview.innerHTML = \"Write review\";";
        }else{            
            echo   "showEditReview.click();
                    if(flipBool){ showNewReview.innerHTML = \"Edit review\"; 
                    }else{ showNewReview.innerHTML = \"Close\"; }
                    flipBool = !flipBool;};showNewReview.innerHTML = \"Edit review\";";
        }    
        echo   "</script>";
